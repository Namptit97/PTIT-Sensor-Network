...
const char* password = "xuanhoang96";
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
           ...
           for (int i = 0 ; i < 256 ; i ++) {
                char a[100],b[100];
                sprintf(a , "GET /9/%d/pwm", i);
                sprintf(b , "GET /10/%d/pwm", i);
                if( header.indexOf(a) >=0) {

                  Serial.print("led Get ");
                  Serial.println(i);
                  analogWrite(output5 , i);
            ...
            client.println("<style>html { font-family: Helvetica; display: inline-block; margin: 0px auto; text-align: center;background-color:#4a7bb3;}");
            client.println(".button { background-color: #00FF00; border: none; color: white; padding: 16px 40px;");
            client.println("text-decoration: none; font-size: 30px; margin: 2px; cursor: pointer;}");
            
            client.println(".button:hover {background-color: yellow;font-size: 40px}");
            ...
            // Web Page Heading
            client.println("<body><h1>VU HAI LONG B15DCDT121</h1>");
            ...
            client.println("xhr.open('GET', \"9/\" + led_output.innerHTML+\"/pwm\", true);");
            client.println("xhr.send();}");
            client.println("coi_slider.oninput = function() { coi_output.innerHTML = this.value;");       
            client.println("xhr.open('GET', \"10/\" + coi_output.innerHTML+\"/pwm\", true);");
